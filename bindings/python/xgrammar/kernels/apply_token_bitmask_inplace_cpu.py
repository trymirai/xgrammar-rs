...
def apply_token_bitmask_inplace_cpu(
    ...
    logits_stride = (logits_stride[0], 1) if logits.dim() == 1 else (logits_stride[0], logits_stride[1])

    bitmask_shape = (1, bitmask.shape[0]) if bitmask.dim() == 1 else (bitmask.shape[0], bitmask.shape[1])
    bitmask_stride = bitmask.stride()
    bitmask_stride = (bitmask_stride[0], 1) if bitmask.dim() == 1 else (bitmask_stride[0], bitmask_stride[1])
