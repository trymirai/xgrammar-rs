...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut encoded_vocab: Vec<String> =
        (0u8..=255).map(|byte| char::from_u32(u32::from(byte)).unwrap().to_string()).collect();
    encoded_vocab.push("</s>".to_string());

    let tokenizer_info = TokenizerInfo::new(&encoded_vocab, VocabType::ByteFallback, None, Some(vec![256]), false);
    ...
    assert!(matcher.accept_string(sample), "grammar rejected: {sample}");
    println!("{sample}");
    println!("completed={} terminated={}", matcher.is_completed(), matcher.is_terminated());
    Ok(())
}
