...
use super::{CompactFsm, CompactFsmWithStartEnd, CompactFsmWithStartEndWithSize, FsmEdge};
use crate::grammar::DeserializeError;

fn ends_to_indices(ends: &[bool]) -> Vec<i32> {
    ends.iter().enumerate().filter_map(|(state, &accepting)| accepting.then_some(state as i32)).collect()
}
...
fn indices_to_ends(
    ...
    for &state in indices {
        ...
        if idx >= ends.len() {
            return Err(DeserializeError::Format(format!("end state {state} out of range for {num_states} states")));
        }
        ends[idx] = true;
    }
    Ok(ends)
...
pub fn deserialize_compact_2d_fsm_edges(
    value: &Value
) -> Result<crate::support::Compact2dArray<FsmEdge>, DeserializeError> {
    deserialize_edges(value)
}

fn deserialize_edges(value: &Value) -> Result<crate::support::Compact2dArray<FsmEdge>, DeserializeError> {
    let obj =
        value.as_object().ok_or_else(|| DeserializeError::Format("compact fsm edges must be an object".to_owned()))?;
    let data = obj.get("data_").ok_or_else(|| DeserializeError::Format("missing compact fsm data_".to_owned()))?;
    let indptr = i32_array(
        obj.get("indptr_").ok_or_else(|| DeserializeError::Format("missing compact fsm indptr_".to_owned()))?,
    )?;
    let rows =
        data.as_array().ok_or_else(|| DeserializeError::Format("compact fsm data_ must be an array".to_owned()))?;
    ...
    for state in 0..indptr.len().saturating_sub(1) {
        ...
        let row: Result<Vec<FsmEdge>, DeserializeError> = rows[start..end].iter().map(parse_edge).collect();
        edges.push_row(&row?);
    }
    Ok(edges)
}
...
fn parse_edge(value: &Value) -> Result<FsmEdge, DeserializeError> {
    let arr = value.as_array().ok_or_else(|| DeserializeError::Format("fsm edge must be an array".to_owned()))?;
    if arr.len() != 3 {
        return Err(DeserializeError::Format("fsm edge must have three elements".to_owned()));
    }
...
impl CompactFsm {
    ...
    pub fn deserialize_json_value(value: &Value) -> Result<Self, DeserializeError> {
        let obj =
            value.as_object().ok_or_else(|| DeserializeError::Format("compact fsm must be an object".to_owned()))?;
        let edges = obj.get("edges").ok_or_else(|| DeserializeError::Format("missing compact fsm edges".to_owned()))?;
        let edge_aux_data = i32_array(
            obj.get("edge_aux_data")
                .ok_or_else(|| DeserializeError::Format("missing compact fsm edge_aux_data".to_owned()))?,
        )?;
...
impl CompactFsmWithStartEnd {
    ...
    pub fn deserialize_json_value(value: &Value) -> Result<Self, DeserializeError> {
        let arr = value
            .as_array()
            .ok_or_else(|| DeserializeError::Format("compact fsm with start/end must be an array".to_owned()))?;
        if arr.len() != 5 {
            return Err(DeserializeError::Format("compact fsm with start/end must have five elements".to_owned()));
        }
        let fsm = CompactFsm::deserialize_json_value(&arr[0])?;
        let start = i32_of(&arr[1])?;
        let end_indices = i32_array(&arr[2])?;
        let is_dfa = arr[3].as_bool().ok_or_else(|| DeserializeError::Format("expected is_dfa boolean".to_owned()))?;
        let _ = i32_of(&arr[4])?;
...
impl CompactFsmWithStartEndWithSize {
    ...
    #[must_use]
    pub fn serialize_json_value(&self) -> Value {
        json!([self.fsm().serialize_json_value(), self.edge_num(), self.node_num(),])
    }
    ...
    pub fn deserialize_json_value(value: &Value) -> Result<Self, DeserializeError> {
        let arr = value
            .as_array()
            .ok_or_else(|| DeserializeError::Format("compact fsm with start/end/size must be an array".to_owned()))?;
        if arr.len() != 3 {
