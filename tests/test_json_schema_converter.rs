...
fn test_generate_float_regex() {
    ...
    assert_eq!(
        generate_float_range_regex(None, None).unwrap(),
        r"^-?\d+(\.\d{1,6})?$"
    );
    ...
    assert_eq!(
        generate_float_range_regex(Some(10.5), Some(2.5)).unwrap(),
        r"^()$"
    );

    assert_eq!(
        generate_float_range_regex(Some(5.123456), Some(5.123457)).unwrap(),
        r"^(5\.123456|5\.123457)$"
    );
